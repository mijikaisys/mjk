...
getgenv().Fsploit = {
    ...
    ["PredictionTime"] = 0.1, -- temps pour prédire la position future de la balle
}
...
local function predict_ball_position(ball)
    local future_position = ball.Position + (ball.AssemblyLinearVelocity * FsploitTable.PredictionTime)
...
task.spawn(function()
    RunService.PreRender:Connect(function()
        ...
        -- Prévoir la position de la balle
        local predictedPosition = predict_ball_position(Ball)

        if (local_player:DistanceFromCharacter(predictedPosition) <= FsploitTable.DistanceToParry) and IsTheTarget() and FsploitTable.AutoParry then
